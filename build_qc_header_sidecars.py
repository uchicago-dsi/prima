...
and reads only DICOM headers (no pixel arrays) from:
...
def build_sidecars(export_dir: Path, raw_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not view_rows:
        raise RuntimeError("no QC sidecars could be built from the supplied export/raw dirs")
